...
/**
 * REST controller for managing {@link com.sidbisht.jhip.defecttracker.domain.Ticket}.
 */
@RestController
@RequestMapping("/api")
public class TicketResource {
    ...
    */
    ...
    /**
     * {@code GET  /tickets/self} : get the ticket assigned to the loggedInUser.
     *
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the ticket, or with status {@code 404 (Not Found)}.
     */
    @GetMapping("/tickets/self")
    public ResponseEntity<List<Ticket>> getMyTicket(Pageable pageable) {
    	Page<Ticket> page  = ticketQueryService.findLoggedInUserTickets(pageable);
    	log.debug("total tickets assigned to User is: {}", page.getContent().size());
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
